...
export default function EditLoanModal({ loan, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Loan for ${loan.employee_name} updated.`, 'Loan Updated');
      onSave(data);
      onClose();
    } catch (err) {
      ...
      {loan && (
        <form className="fg" onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, background: 'var(--bg-tertiary)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', padding: '12px 16px' }}>
            <div>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 3 }}>Loan Amount</div>
            ...
            <div>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 3 }}>Total Installments</div>
              <div style={{ fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>
          ...
          <Input label="Monthly Deduction (PKR)" type="number" min="1" value={form.monthly_deduction} onChange={set('monthly_deduction')} />
          <Input label="Paid Installments" type="number" min="0" max={loan.total_installments} value={form.paid_installments} onChange={set('paid_installments')} />
